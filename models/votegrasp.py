...
class VoteGraspStage1(nn.Module):
...
class VoteGraspStage2(nn.Module):
...
class VoteGrasp(nn.Module):
    def __init__(self, input_feature_dim=0, num_view=300, num_angle=12, num_depth=4, cylinder_radius=0.05, hmin=-0.02, hmax_list=[0.01,0.02,0.03,0.04], is_training=True):
        super().__init__()
        self.is_training = is_training
        self.view_estimator = VoteGraspStage1(input_feature_dim, num_view)
        self.grasp_generator = VoteGraspStage2(num_angle, num_depth, cylinder_radius, hmin, hmax_list, is_training)
